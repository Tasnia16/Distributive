...
selectedFile: File | null = null;
  ...
  onUpload() {
    if (this.selectedFile) {
      ...
      this.http.post<any>('http://localhost:4002/api/v1/images/upload', formData).subscribe(
        (response) => {
...
//http://localhost:9000/routes/post/post
  submitPost() {
    ...
    if (this.selectedFile) {
      ...
      this.http.post<any>('http://localhost:4002/post', formData).subscribe(
        (response) => {
          console.log(response);
          this.post.content = '';
      ...
      this.http.post('http://localhost:4002/post', formData)    // header user nam mail
